<template>
  <el-aside class="index-aside" height="100vh" width="210px">
    <div class="index-aside-inner menulist" style="height:100%">
      <div v-for="item in menuList" :key="item.roleName" v-if="role==item.roleName" class="menulist-item" style="height:100%;broder:0;background-color:#FEFEFE">
        <div class="menulistImg" style="backgroundColor:#D91715;padding:25px 0" v-if="false && menulistStyle == 'vertical'">
        ...
        <el-menu mode="vertical" :unique-opened="false" class="el-menu-demo" style="height:100%;" background-color="#FEFEFE" text-color="#0B0B0B" active-text-color="#8B15DA" default-active="0">
      ...
      this.$nextTick(()=>{
        document.querySelectorAll('.menulist .el-menu-item').forEach(el=>{
          el.addEventListener("mouseenter", e => {
            e.stopPropagation()
            el.style.backgroundColor = "rgba(217, 23, 21, 1)"
          })
